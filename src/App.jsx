...
function App() {
  ...
  // Save conversations to localStorage when they change (NO API calls here)
  useEffect(() => {
  ...
  // --- MAIN API CALL (Optimized & Protected) ---
  const handleSend = async (e) => {
    if (e) e.preventDefault();
    
    // RULE 3 & 6: Safety checks (Lock & Cooldown)
    if (!input.trim() || isLoading || cooldown) return;

    const userText = input.trim();
    setInput(''); // Clear input immediately to prevent double-submit
    setErrorStatus(null);
    ...
    try {
      ...
      // RULE 5: Send ONLY the latest message to save tokens and avoid quota waste
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`,
        {
          ...
          body: JSON.stringify({
            contents: [{ parts: [{ text: userText }] }], // Single payload rule
      ...
      // RULE 7: Safe response handling & Error detection
      if (!response.ok) {
      ...
      if (data.candidates?.[0]?.content?.parts?.[0]?.text) {
        ...
        // RULE 6: Start cooldown period (3 seconds)
        setCooldown(true);
      ...
      if (error.message === 'QUOTA_EXCEEDED') {
        setErrorStatus('⚠️ API limit reached. Please wait 60 seconds and try again.');
      } else {
        setErrorStatus(`⚠️ Error: ${error.message}`);
      }
      // Keep user's message but don't add bot response
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className={`flex h-screen overflow-hidden transition-colors duration-300 ${isDarkMode ? '' : 'light-mode'}`} style={{ backgroundColor: 'var(--color-main-bg)', color: 'var(--color-text-main)' }}>
      ...
      {/* SIDEBAR */}
      <aside className="w-64 flex flex-col border-r border-[var(--color-border-main)]" style={{ backgroundColor: 'var(--color-sidebar-bg)' }}>
        <div className="p-6">
        ...
        <div className="flex-1 overflow-y-auto hide-scrollbar px-4 space-y-1">
          ...
          {conversations.map((chat) => (
            <div key={chat.id} onClick={() => loadChat(chat)} className={`group flex items-center justify-between px-3 py-2.5 text-sm rounded-lg cursor-pointer transition-all ${currentChatId === chat.id ? 'bg-blue-500/10 text-blue-500 border border-blue-500/20' : 'text-gray-500 hover:text-blue-500 hover:bg-gray-500/5'}`}>
              ...
              <button onClick={(e) => deleteChat(e, chat.id)} className="opacity-0 group-hover:opacity-100 p-1 hover:text-red-500 transition-all cursor-pointer"><span className="material-symbols-outlined text-[16px]">delete</span></button>
            </div>
          ))}
        </div>
      ...
      {/* MAIN CONTENT */}
      <main className="flex-1 flex flex-col relative" style={{ backgroundColor: 'var(--color-main-bg)' }}>
        <header className="h-16 flex items-center justify-between px-8 border-b border-[var(--color-border-main)] bg-[var(--color-main-bg)]/80 backdrop-blur-md">
          <div className="flex items-center gap-4">
            <h2 className="text-sm font-bold truncate max-w-[300px]">{conversations.find(c => c.id === currentChatId)?.title || 'New Conversation'}</h2>
            <span className="text-[10px] bg-blue-500/10 text-blue-500 px-2 py-0.5 rounded font-bold border border-blue-500/20">Gemini 2.5</span>
          </div>
        </header>

        {/* CHAT AREA */}
        <div className="flex-1 overflow-y-auto px-4 md:px-0 py-8 hide-scrollbar">
          <div className="max-w-3xl mx-auto space-y-6">
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-[60vh] text-center opacity-40">
                <span className="material-symbols-outlined text-6xl mb-4 text-blue-500">forum</span>
                <h3 className="text-xl font-bold mb-2">How can I help you today?</h3>
              </div>
            ) : (
              messages.map((msg, idx) => (
                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`relative max-w-[85%] group rounded-2xl px-5 py-4 shadow-sm ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-none' : 'rounded-tl-none border border-[var(--color-border-main)]'}`} style={msg.role === 'bot' ? { backgroundColor: 'var(--color-bubble-bot)', color: 'var(--color-text-main)' } : {}}>
                    {msg.role === 'bot' && (
                      <button onClick={() => copyToClipboard(msg.text, idx)} className="absolute top-2 right-2 p-1 bg-gray-500/10 hover:bg-gray-500/20 rounded-md opacity-0 group-hover:opacity-100 transition-all cursor-pointer">
                        <span className="material-symbols-outlined text-[12px]">{copiedIndex === idx ? 'check' : 'content_copy'}</span>
                      </button>
                    )}
                    <div className="prose prose-sm max-w-none" style={{ color: 'inherit' }}>
                      <ReactMarkdown remarkPlugins={[remarkGfm]} components={{ code({node, inline, className, children, ...props}) { const match = /language-(\w+)/.exec(className || ''); return !inline && match ? ( <CodeBlock language={match[1]}>{String(children)}</CodeBlock> ) : ( <code className={`${className} bg-gray-500/10 px-1 rounded`} {...props}>{children}</code> ); } }}>
                        {msg.text}
                      </ReactMarkdown>
                    </div>
                  </div>
        ...
        {/* INPUT AREA (With Lock Protection) */}
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t pt-12 pb-6 px-4" style={{ backgroundImage: `linear-gradient(to top, var(--color-main-bg) 0%, var(--color-main-bg) 70%, transparent 100%)` }}>
          <div className="max-w-3xl mx-auto">
            {/* RULE 7: Quota Error Display */}
            {errorStatus && (
              <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-center">
                <p className="text-xs font-bold text-red-500">{errorStatus}</p>
              </div>
            )}
            
            <div className={`relative border transition-all p-2 pr-3 rounded-[26px] shadow-2xl ${isLoading || cooldown ? 'opacity-50 grayscale pointer-events-none' : 'focus-within:border-blue-500'}`} style={{ backgroundColor: 'var(--color-bubble-bot)', borderColor: 'var(--color-border-main)' }}>
              <div className="flex items-end gap-1">
                <button type="button" className="p-3 text-gray-400 hover:text-blue-500 cursor-pointer shrink-0"><span className="material-symbols-outlined text-[24px]">add</span></button>
                <textarea 
                  className="flex-1 bg-transparent border-none focus:ring-0 resize-none py-3 px-1 text-[15px] leading-[1.5] outline-none max-h-[150px] hide-scrollbar overflow-y-auto" 
                  placeholder={cooldown ? "Please wait..." : "Message Simple AI Chat..."}
                  rows={1}
                  value={input}
                  onChange={(e) => setInput(e.target.value)}
                  onKeyDown={handleKeyDown}
                  disabled={isLoading || cooldown} // RULE 4: Disable during request
                  onInput={(e) => { const target = e.target; target.style.height = '44px'; target.style.height = `${Math.min(target.scrollHeight, 150)}px`; }}
                  style={{ height: '44px', color: 'var(--color-text-main)' }}
                />
                <div className="flex items-center gap-1 mb-1.5 shrink-0">
                  <button type="button" className="p-2 text-gray-400 hover:text-blue-500 cursor-pointer"><span className="material-symbols-outlined text-[20px]">mic</span></button>
                  <button 
                    onClick={handleSend}
                    disabled={!input.trim() || isLoading || cooldown} // RULE 4 & 6: Lock and Cooldown
                    className={`w-8 h-8 flex items-center justify-center rounded-full transition-all cursor-pointer ${input.trim() && !isLoading && !cooldown ? (isDarkMode ? 'bg-white text-black' : 'bg-blue-600 text-white') : 'bg-gray-500/20 text-gray-500'}`}
                  >
                    <span className="material-symbols-outlined text-[20px] font-bold">{isLoading ? 'hourglass_top' : 'arrow_upward'}</span>
                  </button>
                </div>
              </div>
            </div>
            <p className="text-[11px] text-center text-gray-500 mt-3 font-medium">
              {cooldown ? "Wait 3 seconds between messages." : "Simple AI Chat can make mistakes. Check important info."}
            </p>
          </div>
        </div>
      </main>
